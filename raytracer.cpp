    ...
    for (int y = 0; y < ny; ++y) {
      ...
      //   }
        ...
        for (const parser::Mesh & mesh: scene.meshes) {
          float t = intersect_mesh(mesh, scene.vertex_data, r);
          ...
          if (t > 0) {
            ...
            parser::Vec3f mesh_diffuse_color = material.diffuse;
            image[i++] = static_cast < unsigned char > (mesh_diffuse_color.x * 255);
            image[i++] = static_cast < unsigned char > (mesh_diffuse_color.y * 255);
            image[i++] = static_cast < unsigned char > (mesh_diffuse_color.z * 255);
          } else {
            image[i++] = static_cast < unsigned char > (scene.background_color.x * 255);
            image[i++] = static_cast < unsigned char > (scene.background_color.y * 255);
            image[i++] = static_cast < unsigned char > (scene.background_color.z * 255);
